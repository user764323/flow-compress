...
class ImageClassificationExperiment(BaseExperiment):
    """Distillation experiment for image classification tasks."""
    ...
    def load_teacher_model(self) -> nn.Module:
        """Load teacher model."""
        return self._load_model(self.teacher_model_name, pretrained=True)

    def load_student_model(self) -> nn.Module:
        """Load student model."""
        return self._load_model(self.student_model_name, pretrained=False)

    def _load_model(self, model_name: str, pretrained: bool) -> nn.Module:
        try:
            if any(keyword in model_name.lower() for keyword in ["vit", "efficientnet", "convnext"]):
